...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description='Arguments to Get Confidence Map of PoseC3D Model')
...
def pose_inference(args, frame_paths,det_results,pose_model,out_confmap_path): 
    ...
    for i, (f, d) in enumerate(zip(frame_paths, det_results)):
        ...
        for j_idx in range(17):
            ...
            for j, item in enumerate(pose):
                ...
                for y in range(h):
                    for x in range(w):
                        ...
                        conf_value = np.exp(-((y-kp_y)**2+(x-kp_x)**2)/(2*sigma**2))*kp_conf*box_conf
                        confidence_map[y,x] = np.max((confidence_map[y,x],conf_value))
            ...
            part_out_confmap_file = os.path.join(part_out_confmap_path,os.path.split(f)[1])

            confidence_map = (confidence_map*255).astype(np.uint8)
            confidence_total_map = np.max((confidence_total_map,confidence_map),axis=0)
        ...
        cv2.imwrite(os.path.join(total_out_confmap_path,os.path.split(f)[1]),confmap_img)
...
def main():
    ...
    for video_name in os.listdir(args.video_folder):
        #Get Confidence Map
        out_confmap_path = os.path.join(args.video_out_folder,'confmap_'+video_name[:-4])

        if not os.path.exists(out_confmap_path):
            os.mkdir(out_confmap_path)

        _,frame_paths = frame_extraction(osp.join(args.video_folder,video_name)) #return frame

        det_results = detection_inference(args,frame_paths,det_model)
        pose_inference(args,frame_paths,det_results,pose_model,out_confmap_path)
